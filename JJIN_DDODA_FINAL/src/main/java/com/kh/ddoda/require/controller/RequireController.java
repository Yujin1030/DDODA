...
//			System.out.println(folder +","+ userId +","+ folder +","+ filePath);
		...
		//마이페이지 내가 쓴 글 
		@RequestMapping(value="requireContentsList.doa", method=RequestMethod.GET)
		public ModelAndView requireContentsList(ModelAndView mv, 
						@RequestParam(value="userId", required=false) String userId,
						@RequestParam(value="page", required=false)Integer page) {
			int currentPage = (page != null) ? page : 1;
			int listCount = rService.getListCount();
			PageInfo pi = Pagination.getPageInfo(currentPage, listCount);
			ArrayList<Require> requireList = rService.requireContentsList(userId, pi);
			if(!requireList.isEmpty()) {
				mv.addObject("requireContents", requireList);
				mv.addObject("pi", pi);
				mv.setViewName("myPage/requireContents");
			} else {
				mv.addObject("requireContents", requireList);
				mv.addObject("pi", pi);
				mv.setViewName("myPage/requireContents");
