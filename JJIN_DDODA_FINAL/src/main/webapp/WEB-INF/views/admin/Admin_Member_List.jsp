	...
	<div class="main">
		...
		<table align="center" width="800" cellspacing="0" style="clear:right;" class="table">
			...
			<c:forEach items="${mList }" var="member">
				<c:url var="mDetail" value="adminMemberDetail.doa">
					<c:param name="userId" value="${member.userId }"/>
					<c:param name="page" value="${pi.currentPage }"/>
				</c:url>
				<tr>
					<td align="center">${member.userId }</td>
					<td align="center" onclick="location.href='${mDetail }'">${member.userName }</td>
					<td align="center">${member.userField }</td>
					<td align="center">
						<c:if test="${member.memStatus == 'Y'}">
							<c:url var="memberDelete" value="adminMemberDelete.doa">
								<c:param name="userId" value="${member.userId }"/>
							</c:url>
							<a href="${memberDelete }" class="insert btn btn-primary">탈퇴하기</a>
						</c:if>
						<c:if test="${member.memStatus == 'N'}">
							<button class="insert btn btn-primary" disabled>탈퇴불가</button>
						</c:if>
					</td>
				</tr>
			</c:forEach>
