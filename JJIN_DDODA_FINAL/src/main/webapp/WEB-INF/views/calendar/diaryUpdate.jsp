...
<!-- header -->
	...
	<h3 align="center">${opendiary.opendiaryDate }, 오늘의 일기</h3>
	<form id="fileForm" action="myDiaryModify.doa" method="post" name="opendiaryForm"  enctype="multipart/form-data"  onsubmit="return checkIt()">
		...
		<table class="table">
			<tr>
				<td>일기를 공개하시겠습니까? <input type="radio" id="openYn" name="openYn" value="Y" <c:if test="${opendiary.openYn eq 'Y' }">checked</c:if> />네 
					<input type="radio" id="openYn" name="openYn" value="N" <c:if test="${opendiary.openYn ne 'Y' }">checked</c:if> />아니요</td>
			</tr>
			<tr>
			...
			<tr>
				<td>
					<div>첨부파일<br>(첨부된 라이브러리 내 사진 포함, 총 5개까지 업로드 가능합니다)</div>
