...
<body>
	...
	<div class="main-name">
		<h2 class="main-name-h2">메이트 모집글 작성</h2>
	</div>
	<div class="table-wrap">
	<form name="fr" action="mateInsert.doa" method="post"  onsubmit="return submitCheck();">
		<table align="center" id="tb" class="table">
			<tr>
				<td>제목</td>
				<td><input type="text" name="mateTitle" placeholder="제목을 입력해주세요"></td>
			</tr>
			<tr>
				<td>작성자</td>
			...
			<tr>
				...
				<td>
					<select id="mateRequireNo" name="mateRequireNo">
						<option value="1|3" >1~3명</option>
						<option value="3|5" >3~5명</option>
						<option value="5|7" >5~7명</option>
						<option value="7|9" >7~9명</option>
					</select>
				</td>
			<tr>
				<td>내용</td>
	...
	$(document).ready(function() {
		...
		$('form[name="fr"]').bind('submit', function() {
			if($('input[name="mateTitle"]').val() == '') {
				alert("제목을 입력해주세요");
				$('input[name="mateTitle"]').focus();
				return false;
			} else if($('input[name="matePlace"]').val() == '') {
				alert("모임 장소를 검색해주세요");
				$('input[name="matePlace"]').focus();
				return false;
			} else if ($('#mateRequireNo option:selected').val() == '') {
				alert("모집 인원을 선택해주세요");
				return false;
