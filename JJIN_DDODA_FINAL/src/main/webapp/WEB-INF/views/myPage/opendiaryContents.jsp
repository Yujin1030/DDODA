				...
				<table align="center" cellpadding="10" cellspacing="0" width="500" class="table" id="mymateList">
					...
					<c:forEach items="${ opendiaryContents }" var="opendiaryContents" varStatus="status">
						...
						<tr onclick="location.href='${openDetail}'">
							...
							<c:if test="${ opendiaryContents.mainImagePath != null}">
								<td>O</td>
							</c:if>
							<c:if test="${ opendiaryContents.mainImagePath == null }">
								<td>X</td>
				...
				<tr align="center">
					<td colspan="7">
						<!-- 이전 --> <c:if test="${pi.currentPage <= 1}"> < </c:if> <c:if
							...
							<c:url var="before" value="opendiaryContentsList.doa">
							...
							<c:url var="pagination" value="opendiaryContentsList.doa">
								<c:param name="userId" value="${loginUser.userId}"></c:param>
						...
						<c:if test="${pi.currentPage < pi.maxPage}">
							<c:url var="after" value="opendiaryContentsList.doa">
								<c:param name="userId" value="${loginUser.userId}"></c:param>
								<c:param name="page" value="${pi.currentPage +1 }"></c:param>
							</c:url>
							<a href="${after }"> > </a>
